...
export function AuthForm({ mode }: AuthFormProps) {
  ...
  const { signInWithEmail, signUpWithEmail, signInWithGoogle, loading, error, clearError } =
    useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      result = await signUpWithEmail(email, password, displayName || undefined);
    }
      ...
      <div className="bg-zinc-800 border border-zinc-700 rounded-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full px-4 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
